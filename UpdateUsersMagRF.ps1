...
$BWADPath = 'OU=РФ,OU=Сотрудники магазинов,OU=Филиалы,OU=ТУП Белвест Ритейл,DC=belwest,DC=corp'
$BWADPathNewUsers = 'OU=РФ,OU=Сотрудники магазинов,OU=Филиалы,OU=ТУП Белвест Ритейл,DC=belwest,DC=corp'
...
# файлы логов
$BadOutFile=$Logfilepath+'Магазины РФ Не обработаные '+(Get-Date  -Format "yyyyMMdd")+'.txt'
$NewGroupFile=$Logfilepath+'Магазины РФ Новые группы '+(Get-Date  -Format "yyyyMMdd")+'.txt'
$NewUserFile=$Logfilepath+'Магазины РФ Изменения у пользователей '+(Get-Date  -Format "yyyyMMdd")+'.txt'
$UserErrorFile=$Logfilepath+'Магазины РФ ошибки обработки '+(Get-Date  -Format "yyyyMMdd")+'.txt'
$NewAccountFile=$Logfilepath+'Новые учетные записи '+(Get-Date  -Format "yyyyMMdd")+'.txt'

$FullLog=$Logfilepath+(Get-Date  -Format "yyyyMMdd")+' Магазины РФ ImportLog.txt'
...
Out-File $NewAccountFile -InputObject ("--------------------Магазины РФ--------------------") -Append -Encoding "Default"
...
$csv | Select-Object CodeOrg, Org, FIO, ProfFull, DepatFull, Tubnum, Prof, Depat, Birthday, EmploymentDate, Familia, Name, Otch | ForEach-Object{
	$empDate = $null
	$CodeOrg = $_.CodeOrg.Substring(2)
	$empType = "РФ Магазин"
	$FIO = $_.FIO
	$TubNum = $_.TubNum
	$birthday = $_.Birthday
	$magazin = ($_.Depat -match "\d+") | %{$matches[0]}
	...
	if($empDate -eq $null) {Out-File $UserErrorFile -InputObject ($_.FIO + " - некорректная дата приема на работу: " + $_.EmploymentDate) -Append -Encoding "Default"}
		If ($CodeOrg -eq "Y1")
			...
			$user = get-ADUser -Searchbase $BWADPath -Filter {EmployeeID -eq $TubNum} -Properties EmployeeID, Department, Title, Company, Description, fullDeparment, comment, birthDay, employeeType, info, surname
			...
			else
			{
				...
				{
					$transLastName=Translit($_.Familia)
					...
					New-ADUser -SamAccountName $samAccountName `
						...
						-OtherAttributes @{'EmployeeID'=$_.TubNum;'birthDay'=$_.BirthDay;'physicalDeliveryOfficeName'=$newPass;'info'=$magazin} `
						-Enabled $true
					...
					if ($_.Depat -ne '')
					{
						Add-ADGroupMember -Identity ("shopru"+$Familia.substring(0,2)) -Members $NewUser
					}
					Add-ADGroupMember -Identity ("Shop Ru") -Members $NewUser
					Out-File $NewAccountFile -InputObject ($_.FIO+", "+$_.ProfFull+", "+$_.Depat+", №"+$_.TubNum+", "+$samAccountName) -Append -Encoding "Default"
				}
				else
				{
					Out-File $badoutfile -InputObject ($_.FIO+" с №"+$_.TubNum+" не найден в AD в OU Ритейл Филиалы РФ") -Append -Encoding "Default" 
					Out-File $FullLog -InputObject ($_.FIO+" с №"+$_.TubNum+" не найден в AD в OU Ритейл Филиалы РФ") -Append -Encoding "Default"
				}
		...
		{
			Out-File $FullLog -InputObject ($_.FIO+" с №"+$_.TubNum+" не является сотрудником сети магазинов РФ"+$ADGroup) -Append -Encoding "Default"
		}
} | Out-Null
